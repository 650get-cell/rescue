...
// Save full state (admin writes everything EXCEPT availability).
// Availability is owned by the crew submission endpoint and the admin-set endpoint below.
// This prevents a stale admin browser from wiping crew submissions that arrived
// after the admin's page loaded (the "only the last submission survives" bug).
app.put('/api/state', requireAdmin, (req, res) => {
  ...
  // Keep the server's availability, not the client's stale copy
  newState.availability = diskState.availability || {};
  // Housekeeping: drop availability records for employees deleted in this update
  const validIds = new Set((newState.employees || []).map(e => e.id));
  for (const monthKey of Object.keys(newState.availability)) {
    for (const empId of Object.keys(newState.availability[monthKey])) {
      if (!validIds.has(parseInt(empId))) delete newState.availability[monthKey][empId];
    }
  }
  saveState(newState);
  res.json({ ok: true, availabilityPreserved: true });
});

// Targeted availability write for admin (used by availability_view's Edit Submission).
// Sets or deletes a single employee/month record without touching anything else.
// monthIndex is 0-based (matches the 'YYYY_M' storage keys).
app.post('/api/availability/admin-set', requireAdmin, (req, res) => {
  const { empId, year, monthIndex, days } = req.body;
  if (!empId || year === undefined || monthIndex === undefined) {
    return res.status(400).json({ error: 'empId, year, monthIndex required' });
  }
  const state = loadState();
  ...
  if (days === null) {
    ...
    if (state.availability[key]) delete state.availability[key][emp.id];
    saveState(state);
    return res.json({ ok: true, deleted: true });
  }

  if (!Array.isArray(days)) return res.status(400).json({ error: 'days must be an array or null' });
  if (!state.availability[key]) state.availability[key] = {};
  const existing = state.availability[key][emp.id];
  const sorted = days.slice().sort((a, b) => a - b);

  if (existing && typeof existing === 'object' && !Array.isArray(existing)) {
    // Preserve the crew-submitted metadata, just update days + audit fields
    existing.days = sorted;
